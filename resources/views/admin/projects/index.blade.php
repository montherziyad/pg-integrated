    ...
    <div class="space-y-6">
        ...
        <div class="pg-card"><div class="pg-card-body overflow-x-auto">
            <table class="w-full text-sm">
                <thead><tr class="border-b text-left text-slate-500"><th class="py-3">Project</th><th>Code</th><th>Client</th><th>Manager</th><th>Dates</th><th>Status</th><th></th></tr></thead>
                <tbody>
                @forelse($projects as $project)
                    <tr class="border-b last:border-b-0">
                        <td class="py-4 font-semibold">{{ $project->name }}</td><td>{{ $project->project_code }}</td>
                        <td>{{ $project->client?->name ?? '-' }}</td><td>{{ $project->projectManager?->name ?? '-' }}</td>
                ...
                @empty<tr><td colspan="7" class="py-10 text-center text-slate-500">No projects found.</td></tr>@endforelse
                </tbody>
            </table>
        </div></div>
    </div>
