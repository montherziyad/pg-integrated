<x-app-layout>
    ...
    <div class="space-y-6">
        ...
        <div class="pg-card"><div class="pg-card-body overflow-x-auto"><table class="w-full text-sm">
            <thead><tr class="border-b text-left text-slate-500"><th class="py-3">Name</th><th>Code</th><th>Jobs</th><th>Status</th><th></th></tr></thead>
            <tbody>@forelse($categories as $category)<tr class="border-b last:border-b-0"><td class="py-4 font-semibold">{{ $category->name }}</td><td>{{ $category->code }}</td><td>{{ $category->jobs_count }}</td><td><span class="pg-badge {{ $category->is_active ? 'pg-badge-completed' : 'pg-badge-review' }}">{{ $category->is_active ? 'Active' : 'Inactive' }}</span></td><td class="text-right"><a href="{{ route('admin.categories.show', $category) }}" class="pg-btn-secondary">View</a></td></tr>@empty<tr><td colspan="5" class="py-10 text-center text-slate-500">No categories found.</td></tr>@endforelse</tbody>
        </table></div></div>
    </div>
</x-app-layout>
